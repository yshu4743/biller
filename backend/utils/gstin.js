...
function checkDigitOk(gstin) {
  let sum = 0;
  for (let i = 0; i < 14; i++) {
    const cv = CHECK_CHARS.indexOf(gstin[i]);
    if (cv < 0) return false;
    const factor = Math.pow(2, i + 1) % 36;
    sum += cv * factor;
  }
  const mod = sum % 36;
...
export function validateGstin(raw) {
  ...
  breakdown.pan = pan;
  if (!/^[A-Z]{5}[0-9]{4}[A-Z]$/.test(pan)) errors.push('Characters 3-12 must be a valid PAN (5 letters, 4 digits, 1 letter)');

  const entityChar = gstin[12];
  breakdown.entityType = entityChar;
  ...
  }[entityChar] || (entityChar >= 'A' && entityChar <= 'Z' ? 'Individual / others' : '');
  if (!/^[1-9A-Z]$/.test(entityChar)) errors.push('Char 13 (entity type) must be a valid entity code (1-9 or A-Z)');
